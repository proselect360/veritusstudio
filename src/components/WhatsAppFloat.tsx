...
export default function WhatsAppFloat() {
  ...
  // Si estamos en el /studio, no renderizamos nada para evitar bloqueos
  const isStudio = pathname?.startsWith('/studio')

  useEffect(() => {
    if (isStudio) return // Si es el studio, no ejecutamos timers ni scroll
  ...
  if (isStudio) return null // No muestra el botón en el panel de Sanity

  return (
    // Subimos el z-index a 9999 para que siempre sea clickeable
    <div className={`fixed bottom-6 right-6 z-[9999] transition-all duration-700 ${
      ...
      <a
        ...
        className="group relative block pointer-events-auto" // Aseguramos que detecte clics
        aria-label="Chatear por WhatsApp"
      >
        {/* ... resto de tu código igual (Glow, SVG, Tooltip) ... */}
